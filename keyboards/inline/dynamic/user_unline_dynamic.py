...
async def get_services_kb(page: list, number: int = 0, pages_count: int = 36):
    buttons = []
    for item in page:
        buttons.append(InlineKeyboardButton(text=f"{item}", callback_data=f"service {item}"))
    ...
    left_button = InlineKeyboardButton("←", callback_data=f"tos {left}")
    page_button = InlineKeyboardButton(f"{number+1}/{pages_count}", callback_data="pagenumber")
    right_button = InlineKeyboardButton("→", callback_data=f"tos {right}")
...
async def get_countries_kb(page: list, number: int = 0, pages_count: int = 18):
    ...
    left_button = InlineKeyboardButton("←", callback_data=f"toc {left}")
    page_button = InlineKeyboardButton(f"{number + 1}/{pages_count}", callback_data="pagenumber")
    right_button = InlineKeyboardButton("→", callback_data=f"toc {right}")
    keyboard.row(left_button, page_button, right_button)
